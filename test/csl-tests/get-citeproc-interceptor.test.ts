...
describe("GetCiteProcInterceptor", function () {
  ...
  it("configures citeproc engines without manual helpers", async function () {
    ...
    const initialLangPrefs =
      engine.opt?.["cite-lang-prefs"]?.persons?.slice() ?? null;
    ...
    assert.isAtLeast(
      citeLangPrefs!.length,
      1,
      "cite-lang-prefs.persons should have entries",
    );
