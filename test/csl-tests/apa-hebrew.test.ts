...
import { serializeToExtra } from "../../src/modules/cne/metadata-parser";
import {
...
for (const language of ["he-IL", "he"]) {
  const suiteName = `APA Hebrew title variants (#13), ${language}`;

  describe(suiteName, function () {
    let item: Zotero.Item;

    before(async function () {
      // Use the serializer called by the sidebar's metadata save operation.
      const extra = serializeToExtra("", {
        title: {
          original: "ספר לדוגמה",
          romanized: "Sefer ledugma",
          english: "An example book",
        },
      });
      item = await createZoteroItemFromTestCase({
        ...HEBREW_BOOK,
        language,
        extra,
      });
